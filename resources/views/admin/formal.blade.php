...
<?php  $codigo = "SELECT s.id, s.fecha_solicitud, t.nom_tiposolicitud, te.nom_tipoexcepcion, o.nom_opciones, s.fechainicio, s.fechafin, s.horainicio, s.horafin, s.comentario, s.fecharespuesta, u.nombres_usu, u.apellidos_usu, u.codigo_usu, g.nom_gerencia, d.nom_depto FROM solicitud AS s, tiposolicitud AS t, tipoexcepcion AS te, opcioneslista AS o, usuario AS u, gerencia AS g, departamento AS d WHERE s.id_usuario=u.id AND s.id_tiposolicitud = t.id_tiposolicitud AND s.id_tipoexcepcion = te.id_tipoexcepcion AND s.id_opcioneslista = o.id_opcioneslista AND u.id_gerencia = g.id_gerencia AND u.id_departamento = d.id_departamento and s.estado = '1' ORDER BY s.fecha_solicitud DESC";
...
<div class="mx-auto max-w-4xl bg-yellow-100 py-5 px-12 lg:px-24 shadow-xl mb-24">
    <form action="{{route('usuario.solpermiso')}}" method="POST" id="formulario" autocomplete="off">
