...
<?php  $codigo = "SELECT s.id, s.fecha_solicitud, t.nom_tiposolicitud, te.nom_tipoexcepcion, o.nom_opciones, s.fechainicio, s.fechafin, s.horainicio, s.historial_remitentes,
                  ...
                  tiposolicitud AS t, tipoexcepcion AS te, opcioneslista AS o, usuario AS u, gerencia AS g, departamento AS d WHERE u.codigo_usu='$codigo1'
                  and u.id =s.id_usuario AND s.id_tiposolicitud = t.id_tiposolicitud AND s.id_tipoexcepcion = te.id_tipoexcepcion AND s.id_opcioneslista = o.id_opcioneslista
...
<div class="mx-auto max-w-4xl bg-yellow-100 py-5 px-12 lg:px-24 shadow-xl mb-24">
    <form action="{{route('usuario.imprimir1')}}" method="POST" id="formulario" autocomplete="off">
    <input type="HIDDEN"  name="iden" size="7" id="iden"value="<?php echo $rest ['id']; ?>">
      ...
      @csrf
        ...
        <label class="uppercase tracking-wide text-black text-xs font-bold mb-2"> Aprobado por:</label>

        <p ><?php echo $rest ['historial_remitentes']; ?> </p>


        <label class="uppercase tracking-wide text-black text-xs font-bold mb-2"> Comentario:</label>
